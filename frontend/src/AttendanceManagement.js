...
const AttendanceManagement = () => {
  ...
  useEffect(() => {
    const loadAttendanceForDate = async () => {
      ...
      try {
        ...
        if (response.success) {
          // Create a map of employee_id to status
          const attendanceMap = new Map(
            response.data.map(record => [
              record.employee.employee_id,  // Access nested employee object
              record.status
            ])
          );
        ...
        // Set employees without attendance status on error
        setAttendanceData(employees.map(emp => ({ ...emp, status: null })));
      } finally {
  ...
  return (
    <div className="attendance-management">
      {successMessage && (
        <div className={`notification ${successMessage.startsWith('Error') ? 'notification-error' : 'notification-success'}`}>
